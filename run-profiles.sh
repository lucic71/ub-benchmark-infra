...
do
	...
	pts_command="echo -n '$result_name' | php /git/phoronix-test-suite/pts-core/phoronix-test-suite.php batch-run $p"
	/usr/bin/time sh -c "$pts_command" 2>&1| tee -a $LOG_FILE
done

# PATH is set for the linker to be found
export PATH=`pwd`"/llvm-project-llvmorg-15.0.7/build/bin:$PATH"
